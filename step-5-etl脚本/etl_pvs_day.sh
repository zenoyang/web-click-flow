...
# 功能描述:     多维度统计PV总量  维度：日
# 输入参数:     运行日期
# 目标表名:     web_click_flow_project.dw_pvs_day
...
TARGET_TABLE=dw_pvs_day

HQL="
insert into table $TARGET_DB.$TARGET_TABLE
select sum(pvs) as pvs,month,day from $TARGET_DB.dw_pvs_hour 
group by month,day having day='$sday';
"
#执行hql
echo "$HQL"
$exe_hive -e "$HQL"
